...
if (uploadBtn) uploadBtn.addEventListener('click', async ()=>{
  ...
  try{
    ...
    if (aiSummaryDiv){
      if (data.ai_summary){
        aiSummaryDiv.innerHTML = '<h3>AI summary</h3><pre>' + escapeHtml(data.ai_summary) + '</pre>'
        aiSummaryDiv.style.display = 'block'
      } else if (data.ai_summary_error){
        aiSummaryDiv.innerHTML = '<h3>AI summary</h3><pre>' + escapeHtml(data.ai_summary_error) + '</pre>'
        aiSummaryDiv.style.display = 'block'
      } else {
